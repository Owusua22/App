  ...
  useFocusEffect(
    React.useCallback(() => {
      const checkScrollRestore = async () => {
        try {
          const savedScrollPosition = await AsyncStorage.getItem("ComboScreenScrollPosition");
  ...
  const handleProductPress = async (productId) => {
    try {
      ...
      if (flatListRef.current) {
        await AsyncStorage.setItem("ComboScreenScrollPosition", currentScrollOffset.toString());
      }
    } catch (error) {
  ...
  const handleShare = async () => {
    try {
      const result = await Share.share({
        message: 'Check out these amazing Combo at great prices!',
