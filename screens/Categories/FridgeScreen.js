...
import { AntDesign, Ionicons, FontAwesome, Octicons } from "@expo/vector-icons";
...
const FridgeScreen = () => {
  ...
  // Once products loaded & shouldRestoreScroll is true
  const filteredProducts = useMemo(() => {
    let products = productsByCategory[hardcodedCategoryId] || [];

    if (selectedSellers.length > 0) {
      products = products.filter((p) =>
        selectedSellers.includes(p.sellerName)
      );
    }
  ...
  const toggleWishlistForProduct = (product) => {
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        message: "Check out these amazing refrigerators at Franko!",
        title: "Refrigerators",
      });
    } catch {
  ...
  if (productsLoading) {
    return (
      <View style={styles.loaderContainer}>
        ...
        <Text style={styles.loaderText}>Loading refrigerators...</Text>
      </View>
    );
  }
  ...
  const renderItem = ({ item, index }) => {
    const imageUri = getValidImageURL(item.productImage);
    const discount =
      item.oldPrice > 0
        ? Math.round(
    ...
    const isNew = index < 3;
    const isAddingThis = addingToCart[item.productID];
      ...
      >
        ...
        <TouchableOpacity
          style={[
            ...
            isAddingThis && styles.addToCartButtonDisabled,
          ]}
          onPress={(e) => {
            e.stopPropagation();
        ...
        >
          {isAddingThis ? (
            ...
            <AntDesign name="shopping-cart" size={14} color="#fff" />
          )}
        </TouchableOpacity>
      </TouchableOpacity>
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Refrigerators</Text>
      ...
      {/* Filter Modal (placeholder for price etc.) */}
      {renderModal(
        filterModalVisible,
