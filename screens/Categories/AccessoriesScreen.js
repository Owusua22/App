...
// Spacing – consistent across category screens
const SPACING = {
  xs: 4,
  sm: 8,
  md: 12,
...
const AccessoriesScreen = () => {
  ...
  // Hardcoded category id for Accessories
  ...
  // Per‑product "add to cart" loading
  ...
  useFocusEffect(
    useCallback(() => {
      const checkScrollRestore = async () => {
        try {
          const saved = await AsyncStorage.getItem(
            "AccessoriesScreenScrollPosition"
          );
          if (saved) {
      ...
      checkScrollRestore();
      return () => setShouldRestoreScroll(false);
    }, [])
  );

  const rawProducts = productsByCategory[hardcodedCategoryId] || [];

  // Unique sellers/brands
  const { uniqueSellers, uniqueBrands } = useMemo(() => {
    const sellers = [
      ...new Set(rawProducts.map((p) => p.sellerName).filter(Boolean)),
    ];
    const brands = [
      ...new Set(rawProducts.map((p) => p.brandName).filter(Boolean)),
    ];
  ...
  // Scroll restore when products ready
  useEffect(() => {
  ...
  const handleScroll = (e) => {
    setCurrentScrollOffset(e.nativeEvent.contentOffset.y);
  };
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        message:
          "Check out these amazing accessories deals at great prices on Franko Trading!",
  ...
  if (productsLoading) {
    return (
      <View style={styles.loaderContainer}>
        ...
        <Text style={styles.loaderText}>Loading accessories...</Text>
      </View>
    );
  }
  ...
  const renderItem = ({ item, index }) => {
    const imageURL = getValidImageURL(item.productImage);
    const discount =
      item.oldPrice > 0
        ? Math.round(
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Accessories Deals</Text>
      ...
      {renderModal(
        ...
        sortOptions.map((opt) => (
          <TouchableOpacity
            ...
            style={[
              styles.modalOption,
              selectedSort === opt.key && styles.modalOptionSelected,
            ]}
            onPress={() => {
